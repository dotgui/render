...
export class GuiCanvas extends HTMLElement {
  ...
  async load(url) {
    ...
    const xml = await step('unpack', async () => isPackage
      ? call('loadPackage', { key: this.#key, bytes }, [bytes])
  ...
  async #render() {
    ...
    try {
      do {
        ...
        if (frame.missingImages > 0) this.#supplyImages(xml, proxy)
      } while (this.#pending)
    } catch (error) {
   ...
   */
  async #supplyImages(xml, proxy) {
    if (this.#supplyingImages) return
    this.#supplyingImages = true
    try {
      const supplied = await call('supplyImages', { key: this.#key, xml, proxy })
