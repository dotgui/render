...
fn main() {
    let path = env::args().nth(1).unwrap_or_else(|| {
        eprintln!("usage: cargo run -p dotgui-renderer --example parse <file.guix>");
        process::exit(2);
    });
    ...
    let xml = if is_gui_package(&path, &bytes) {
        dotgui_renderer::read_gui_package_xml(&bytes).unwrap_or_else(|err| {
    ...
    let document = dotgui_renderer::parse_gui_xml(&xml).unwrap_or_else(|err| {
    ...
    println!(
        "{}",
        serde_json::to_string_pretty(&document).expect("document serializes to json")
    );
}
